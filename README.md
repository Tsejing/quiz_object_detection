...
>可以使用已经准备好的数据集 or 自行制作数据集。  PS:我都做过，上传的是自己制作的小车装甲板检测效果，模型和数据集都传在tinymind平台上
...
**请把相关代码下载本地后，删除不需要的代码。到请勿直接使用这个链接作为tinymind模型的代码。**
...
### 数据准备
模型引用的数据集中，应包含以下文件，文件名需要跟此处文件名一致。
...
为避免因对流程的不熟悉浪费太多的时间，这里提供一些关键流程的要点提示。本作业主要参考：
...
注意要点：
- num_classes， 原文件里面为37,这里的数据集为2（红装甲板和蓝装甲板）
...
###   本人tinymind模型地址
https://www.tinymind.com/executions/pcpbt5d8 感兴趣可以到里面去看我的模型，代码，运行的效果以及输出（ckpt,test.png,etc..）
